...
function detectarPais4(input) {
    ...
    imagenBandera4.src = banderaPorDefecto4;
    imagenBandera4.style.display = "inline";
}
...
window.onload = function() {
    const campoTelefono4 = document.getElementById('vendedor_telefono');
...
function detectarPais5(input) {
    ...
    for (const codigo in banderasPais5) {
        if (numeroTelefono5.startsWith(codigo)) {
            imagenBandera5.src = banderasPais5[codigo];
            imagenBandera5.style.display = "inline"; // Mostrar la imagen de la bandera
            return; // Detener la función si se encuentra el país
        }
    }
    // Si no se encuentra un código de país coincidente, muestra la bandera por defecto
    imagenBandera5.src = banderaPorDefecto5;
    imagenBandera5.style.display = "inline";
}
...
// Asegúrate de que la bandera se actualice al cargar la página
window.onload = function() {
    const campoCelular5 = document.getElementById('vendedor_celular');
    asegurarMasYDetectarPais5(campoCelular5); // Llama a la función para asegurar "+" y detectar el país
};
