...
CREATE TABLE Adelanto (
    ...
    porcentaje_adelanto INT(3) NOT NULL DEFAULT 0, 
    monto_adelanto INT(10) NOT NULL DEFAULT 0,
...
DROP TABLE IF EXISTS Requisitos_Basicos;
CREATE TABLE Requisitos_Basicos (
    id_requisitos INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    primer_titulo VARCHAR(255) NOT NULL,
    descripcion_condiciones VARCHAR(255) NOT NULL,
    estado BOOLEAN DEFAULT FALSE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;

-- Crear la tabla Requisitos_Basicos
DROP TABLE IF EXISTS Obligaciones_Cliente;
CREATE TABLE Obligaciones_Cliente (
    id_obligacion INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    primer_titulo VARCHAR(255) NOT NULL, --nueva tabla?
    descripcion_condiciones VARCHAR(255) NOT NULL,--nueva tabla?
    estado BOOLEAN DEFAULT FALSE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
CREATE TABLE `Cotizaciones` (
  ...
  `sub_total` INT(10) DEFAULT 0, -- Sub Total de la cotización
  `descuento_global` INT(3) NOT NULL DEFAULT 0, -- Descuento Global aplicado a la cotización (porcentaje)
  `monto` INT(10) DEFAULT 0, -- Monto de descuento de la cotización
  `monto_neto` DECIMAL(10,2) NOT NULL DEFAULT 0, -- Monto Neto de la cotización
  `iva_valor` INT(2) NOT NULL DEFAULT 0, -- Valor del IVA aplicado (19%)
  `total_iva` DECIMAL(10,2) NOT NULL DEFAULT 0, -- Total del IVA de la cotización
  `total_final` INT(10) DEFAULT 0, -- Total Final de la cotización
  `id_adelanto` INT, -- ID del adelanto (clave foránea)
  `id_condiciones` INT, -- ID de las condiciones generales (clave foránea)
  `id_requisitos` INT, -- ID de los requisitos básicos (clave foránea)
  ...
  FOREIGN KEY (`id_adelanto`) REFERENCES `Adelanto`(`id_adelanto`) ON DELETE SET NULL, -- Clave foránea hacia Adelanto
  FOREIGN KEY (`id_condiciones`) REFERENCES `Condiciones_Generales`(`id_condiciones`) ON DELETE SET NULL, -- Clave foránea hacia Condiciones_Generales
  FOREIGN KEY (`id_requisitos`) REFERENCES `Requisitos_Basicos`(`id_requisitos`) ON DELETE SET NULL -- Clave foránea hacia Requisitos_Basicos
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
