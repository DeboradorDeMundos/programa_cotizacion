...
CREATE TABLE `Detalle_Cotizacion` (
    ...
    `descripcion` varchar(255) NOT NULL, -- Descripción del ítem o servicio
    ...
    `subtotal` decimal(10,2) AS (cantidad * precio_unitario) STORED, -- Subtotal calculado automáticamente
    `descuento` decimal(10,2) DEFAULT 0.00, -- Descuento aplicado al ítem
    `total` decimal(10,2) AS (subtotal - descuento) STORED, -- Total con descuento calculado automáticamente
    `neto` decimal(10,2) AS (total / 1.19) STORED, -- Neto calculado automáticamente asumiendo IVA del 19%
    `iva` decimal(10,2) AS (total - neto) STORED, -- IVA calculado automáticamente
