...
CREATE TABLE Cuenta_Bancaria (
    ...
    FOREIGN KEY (id_tipocuenta) REFERENCES Tipo_Cuenta(id_tipocuenta),
...
CREATE TABLE Cotizaciones (
    ...
    id_detalle INT-- Sitio Web Creado por ITred Spa.
-- Direccion: Guido Reni #4190
-- Pedro Agui Cerda - Santiago - Chile
-- contacto@itred.cl o itred.spa@gmail.com
-- https://www.itred.cl
...
CREATE TABLE Cuenta_Bancaria (
    ...
    id_tipocuenta INT NOT NULL,
    numero_cuenta VARCHAR(20) NOT NULL,
    celular VARCHAR(20) NOT NULL, -- Cambiado a VARCHAR(20)
    email_banco VARCHAR(255) NOT NULL,
    id_empresa INT NOT NULL,
    FOREIGN KEY (id_banco) REFERENCES Bancos(id_banco),
    FOREIGN KEY (id_tipocuenta) REFERENCES Tipo_Cuenta(id_tipocuenta),
...
CREATE TABLE Cotizaciones (
    ...
    id_vendedor INT, -- ID del vendedor (clave foránea)
    id_encargado INT, -- ID del encargado (clave foránea)
    id_banco INT, -- ID del banco (clave foránea)
    id_cuenta INT, -- ID de la cuenta bancaria (clave foránea)
    lugar_trabajo VARCHAR(255), -- Lugar de trabajo relacionado con la cotización
    contacto_cotizacion VARCHAR(255), -- Contacto relacionado con la cotización
    id_tipo_cotizacion INT NOT NULL, -- ID del tipo de cotización (clave foránea)
    id_estado_cotizacion INT NOT NULL, -- ID del estado de la cotización (clave foránea)
    subtotal_cotizacion DECIMAL(10, 2) DEFAULT 0.00, -- Subtotal de la cotización
    impuestos_cotizacion DECIMAL(10, 2) DEFAULT 0.00, -- Impuestos aplicados a la cotización
    total_cotizacion DECIMAL(10, 2) DEFAULT 0.00, -- Total de la cotización
    observaciones TEXT, -- Observaciones adicionales de la cotización
    fecha_modificacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- Fecha de modificación de la cotización
    PRIMARY KEY (id_cotizacion), -- Clave primaria
    FOREIGN KEY (id_cliente) REFERENCES Clientes(id_cliente) ON DELETE SET NULL, -- Clave foránea con eliminación
    FOREIGN KEY (id_proyecto) REFERENCES Proyectos(id_proyecto) ON DELETE SET NULL, -- Clave foránea con eliminación
    FOREIGN KEY (id_empresa) REFERENCES Empresa(id_empresa) ON DELETE SET NULL, -- Clave foránea con eliminación
    FOREIGN KEY (id_vendedor) REFERENCES Vendedores(id_vendedor) ON DELETE SET NULL, -- Clave foránea con eliminación
    FOREIGN KEY (id_encargado) REFERENCES Encargados(id_encargado) ON DELETE SET NULL, -- Clave foránea con eliminación
    FOREIGN KEY (id_banco) REFERENCES Bancos(id_banco) ON DELETE SET NULL, -- Clave foránea con eliminación
    FOREIGN KEY (id_cuenta) REFERENCES Cuenta_Bancaria(id_cuenta) ON DELETE SET NULL -- Clave foránea con eliminación
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
-- Eliminar la tabla Titulo si existe
...
CREATE TABLE Titulo (
    ...
    numero_titulo int NOT NULL, -- Número del título dentro de la cotización
    nombre_titulo varchar(255) NOT NULL, -- Nombre del título
    PRIMARY KEY (id_titulo), -- Clave primaria
    FOREIGN KEY (id_cotizacion) REFERENCES Cotizaciones(id_cotizacion) ON DELETE CASCADE -- Clave foránea
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
-- Eliminar la tabla Descripcion si existe
...
CREATE TABLE Descripcion (
    ...
    id_cotizacion int NOT NULL, -- ID de la cotización (clave foránea)
    id_titulo int NOT NULL, -- ID del título asociado (clave foránea)
    numero_descripcion int NOT NULL, -- Número de la descripción dentro del título
    nombre_descripcion varchar(255) NOT NULL, -- Nombre de la descripción
    PRIMARY KEY (id_descripcion), -- Clave primaria
    FOREIGN KEY (id_cotizacion) REFERENCES Cotizaciones(id_cotizacion) ON DELETE CASCADE, -- Clave foránea
    FOREIGN KEY (id_titulo) REFERENCES Titulo(id_titulo) ON DELETE CASCADE -- Clave foránea
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
CREATE TABLE Detalle_Cotizacion (
    ...
    id_cotizacion int NOT NULL, -- ID de la cotización (clave foránea)
    id_descripcion int NOT NULL, -- ID de la descripción asociada (clave foránea)
    numero_detalle int NOT NULL, -- Número del detalle dentro de la descripción
    detalle varchar(255) NOT NULL, -- Detalle del ítem
    cantidad int NOT NULL, -- Cantidad del ítem
    precio_unitario decimal(10,2) NOT NULL, -- Precio unitario del ítem
    sub_total decimal(10,2) AS (cantidad * precio_unitario) STORED, -- Subtotal calculado
    PRIMARY KEY (id_detalle), -- Clave primaria
    FOREIGN KEY (id_cotizacion) REFERENCES Cotizaciones(id_cotizacion) ON DELETE CASCADE, -- Clave foránea
    FOREIGN KEY (id_descripcion) REFERENCES Descripcion(id_descripcion) ON DELETE CASCADE -- Clave foránea
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
CREATE TABLE Productos (
    ...
    id_detalle int NOT NULL, -- ID del detalle asociado (clave foránea)
    nombre_producto varchar(255) NOT NULL, -- Nombre del producto
    marca_producto varchar(255), -- Marca del producto
    modelo_producto varchar(255), -- Modelo del producto
    especificaciones_producto text, -- Especificaciones del producto
    PRIMARY KEY (id_producto), -- Clave primaria
    FOREIGN KEY (id_detalle) REFERENCES Detalle_Cotizacion(id_detalle) ON DELETE CASCADE -- Clave foránea
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
-- Eliminar la tabla Adelantos si existe
DROP TABLE IF EXISTS Adelantos;

-- Crear la tabla Adelantos
CREATE TABLE Adelantos (
    id_adelanto int NOT NULL AUTO_INCREMENT, -- Identificador único del adelanto
    id_cotizacion int NOT NULL, -- ID de la cotización (clave foránea)
    numero_adelanto int NOT NULL, -- Número del adelanto dentro de la cotización
    monto_adelanto decimal(10,2) NOT NULL, -- Monto del adelanto
    PRIMARY KEY (id_adelanto), -- Clave primaria
    FOREIGN KEY (id_cotizacion) REFERENCES Cotizaciones(id_cotizacion) ON DELETE CASCADE -- Clave foránea
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
-- ------------------------------------------------------------------------------------------------------------
    ...
    FOREIGN KEY (id_cuenta) REFERENCES Cuenta_Bancaria(id_cuenta) ON DELETE SET NULL -- Clave foránea hacia Cuenta_Bancaria
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
INSERT INTO Tipo_Cuenta (tipocuenta, descripcion) VALUES
