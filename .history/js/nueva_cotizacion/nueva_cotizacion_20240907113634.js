...
/* --------------------------------------------------------------------------------------------------------------
    ...
    document.getElementById('validez_cotizacion').addEventListener('input', calcularFechaValidez);
    document.getElementById('fecha_emision').addEventListener('input', calcularFechaValidez);
    
    function calcularFechaValidez() {
        // Obtiene los valores de los campos de fecha de emisión y validez en días
        const diasValidez = parseInt(document.getElementById('validez_cotizacion').value);
        const fechaEmision = document.getElementById('fecha_emision').value;
    
        if (diasValidez > 0 && fechaEmision) { // Verifica que ambos campos tengan valores válidos
            const fechaEmisionDate = new Date(fechaEmision); // Convierte la fecha de emisión a un objeto Date
            fechaEmisionDate.setDate(fechaEmisionDate.getDate() + diasValidez); // Suma los días de validez
    
            // Formatea la fecha resultante a 'YYYY-MM-DD' para el campo de fecha de validez
            const fechaValidez = fechaEmisionDate.toISOString().split('T')[0];
    
            // Asigna la fecha calculada al campo de fecha de validez
            document.getElementById('fecha_validez').value = fechaValidez;
        } else {
            document.getElementById('fecha_validez').value = ''; // Limpia el campo si los datos no son válidos
        }
    }
    ...
    document.getElementById('agregar-requisito-btn').addEventListener('click', function() {
        // Crear el cuadro de entrada
        const requisito = prompt('Ingrese el nuevo requisito:');
        
        if (requisito) {
            // Crear un nuevo elemento de lista
            const newLi = document.createElement('li');
            newLi.textContent = requisito;
            
            // Agregar el nuevo requisito a la lista
            document.getElementById('requisitos-lista').appendChild(newLi);
        }
    });
