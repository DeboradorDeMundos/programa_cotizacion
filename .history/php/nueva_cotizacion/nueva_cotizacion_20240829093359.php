...
<!-- ------------------------------------------------------------------------------------------------------------
    ------------------------------------- INICIO ITred Spa Formulario Cotizacion .PHP --------------------------------------
    ------------------------------------------------------------------------------------------------------------- -->
...
$sql = "SELECT 
        ...
        f.ruta_foto
    FROM empresa e
    LEFT JOIN FotosPerfil f ON f.id_foto = e.id_foto
    WHERE id_empresa = ?";
...
<head> <!-- Abre el elemento de cabecera que contiene metadatos y enlaces a recursos externos -->
    ...
    <link rel="stylesheet" href="../../css/nueva_cotizacion/formulario_cotizacion.css"> <!-- Enlaza una hoja de estilo externa que se encuentra en la ruta especificada para estilizar el contenido de la página -->
...
<body> <!-- Abre el elemento del cuerpo de la página donde se coloca el contenido visible -->
        ...
        <form id="cotizacion-form" method="POST" action="procesar_cotizacion.php" enctype="multipart/form-data">
            ...
            <div class="row"> <!-- Crea una fila para organizar los elementos en una disposición horizontal -->
                <div class="box-6 logo-box"> <!-- Crea una caja para el logo o foto de perfil, ocupando 6 de las 12 columnas disponibles en el diseño -->
                    ...
                    <label for="logo-upload" class="logo-container"> <!-- Etiqueta para el campo de carga de imagen. El atributo "for" enlaza con el input de archivo -->
                        <?php if (isset($row['ruta_foto']) && !empty($row['ruta_foto'])): ?>
                            <img src="<?php echo htmlspecialchars($row['ruta_foto'], ENT_QUOTES, 'UTF-8'); ?>" alt="Foto de perfil" />
                        <?php else: ?>
                            <p>No se encontró una foto para mostrar.</p>
                        <?php endif; ?>
                ...
                <div class="box-6 data-box data-box-red"> <!-- Crea una caja para ingresar datos, ocupando otras 6 columnas. Se aplica una clase adicional para estilo -->
                    ...
                    <input type="text" id="numero_cotizacion" name="numero_cotizacion" required pattern="\d+" required placeholder="12345" readonly> <!-- Campo de texto para ingresar el número de cotización. También es obligatorio -->
            ...
            <script src="../../js/programa_cotizacion/formulario_cotizacion.js"></script> <!-- Enlaza un archivo JavaScript externo que contiene funciones para manejar la lógica del formulario de cotización -->
    ...
    <fieldset>
        ...
        <table class="detalle-table">
            ...
            <tbody>
                <tr>
                    <td><input type="number" id="sub_total" name="sub_total" step="1" min="0" readonly></td>
                    <td><input type="number" id="descuento_global_porcentaje" name="descuento_global_porcentaje" step="1" min="1" max="100" value="0" oninput="calculateTotals()"></td>
                    <td><input type="number" id="descuento_global_monto" name="descuento_global_monto" step="1" min="0" readonly></td>
                    <td><input type="number" id="monto_neto" name="monto_neto" step="1" min="0" readonly></td>
                    <td><input type="number" id="iva_porcentaje" name="iva_porcentaje" step="0.01" min="0" max="100" value="19" readonly></td>
                    <td><input type="number" id="total_iva" name="total_iva" step="0.01" min="0" readonly></td>
                    <td><input type="number" id="total_final" name="total_final" step="1" min="0" readonly></td>
                </tr>
            </tbody>
        </table>
    </fieldset>
...
<br>
<div id="form-adelanto">
    <label for="descripcion">Descripción:</label>
    <input type="text" id="descripcion" name="descripcion" maxlength="255" required>

    <label for="porcentaje_adelanto">Porcentaje Adelanto: %</label>
    <input type="number" id="porcentaje_adelanto" name="porcentaje_adelanto" min="0" max="100" required oninput="calculateAdelanto()">

    <label for="monto_adelanto">Monto Adelanto:</label>
    <input type="number" id="monto_adelanto" name="monto_adelanto" min="0" required readonly>

    <label for="fecha_adelanto">Fecha Adelanto:</label>
    <input type="date" id="fecha_adelanto" name="fecha_adelanto" required>
</div>
...
<div id="condiciones-generales" class="data-box">
    ...
    <div class="field">
        ...
        <input type="text" id="descripcion_condiciones_1" name="descripcion_condiciones[]" placeholder="Descripción de la condición" required>
        <input type="checkbox" id="estado_condiciones_1" name="estado_condiciones[]">
    </div>
    <!-- Puedes duplicar el bloque anterior para más condiciones generales -->
</div>
        ...
        <div class="box-12 data-box bank-account-container">
            ...
            <div id="bank-accounts">
                ...
                <div class="bank-account">
                    ...
                    <label for="nombre-encargado">Nombre del Encargado:</label>
                    <input type="text" id="nombre-encargado" name="nombre_encargado" required>

                    <label for="rut-banco">RUT del Banco:</label>
                    <input type="text" id="rut-banco" name="rut_banco" 
                        minlength="7" maxlength="12"
                        oninput="formatRut(this)"
                        required>

                    <label for="email-banco">Email del Banco:</label>
...
<script src="../../js/crear_empresa/formulario_cotizacion.js"></script> <!-- Enlaza nuevamente el archivo JavaScript para manejar la lógica del formulario de cotización -->
<script src="../../js/actualizar_logo/actualizar_logo.js"></script>
