...
$sql = "INSERT INTO E_Cuenta_Bancaria (rut_titular, nombre_titular, id_banco, id_tipocuenta, numero_cuenta, celular, email_banco, id_empresa)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);
if ($stmt === false) {
    die("Error en la preparación de la consulta: " . $conn->error);
...
for ($index = 0; $index < $numero_detalles; $index++) {
    ...
    // Inserta el título
    if ($titulo) {
        $sql_insert_titulo = "INSERT INTO C_Titulos (nombre) VALUES (?)";
        ...
        $sql_insert_descripcion = "INSERT INTO C_Descripciones (id_titulo, descripcion, cantidad, precio_unitario) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql_insert_descripcion);
        $stmt->bind_param("isid", $id_titulo, $descripcion, $cantidad, $precio_final);
        $stmt->execute();
        ...
        $sql_insert_detalle = "INSERT INTO C_Detalle_Cotizacion (id_cotizacion, id_descripcion, sub_total, descuento_global, monto, monto_neto, iva_valor, total_iva, total_final) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql_insert_detalle);
        $stmt->bind_param("iiiiididi", $id_cotizacion, $id_descripcion, $sub_total, $descuento_global, $monto, $monto_neto, $iva_valor, $total_iva, $total_final);
        $stmt->execute();
        echo "Detalle de cotización insertado. ID Descripción: $id_descripcion<br>";
    } else {
