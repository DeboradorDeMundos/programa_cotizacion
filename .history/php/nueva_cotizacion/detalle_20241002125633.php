...
<?php
// Suponiendo que ya tienes una conexión a la base de datos establecida en $mysqli

// Ejemplo de datos recibidos (deberías ajustar esto según tus necesidades)
$id_cotizacion = $_POST['id_cotizacion']; // ID de la cotización (debe ser proporcionado)
$detalles_titulo = $_POST['detalles_titulo']; // Array con títulos
$detalles_subtitulo = $_POST['detalles_subtitulo']; // Array con subtítulos para cada título
$detalles_cantidad = $_POST['detalles_cantidad']; // Cantidades de cada detalle
$detalles_precio_unitario = $_POST['detalles_precio_unitario']; // Precios unitarios
$detalles_descuento = $_POST['detalles_descuento']; // Descuentos
$detalles_tipo = $_POST['detalles_tipo']; // Tipos de detalles
$detalles_nombre_producto = $_POST['detalles_nombre_producto']; // Nombres de los productos
$detalles_descripcion = $_POST['detalles_descripcion']; // Descripciones
...
foreach ($detalles_titulo as $titulo_index => $titulo) {
    ...
    foreach ($detalles_cantidad[$titulo_index] ?? [] as $detalle_index => $cantidad) {
        ...
        $subtitulo_index = $detalles_subtitulo[$titulo_index] ? $detalles_subtitulo[$titulo_index][$detalle_index] : null; // Obtener el índice del subtítulo

        $estructura_datos[$titulo_index]['detalles'][] = [
            'subtitulo_index' => $subtitulo_index, // Asociar subtítulo al detalle
...
try {
    foreach ($estructura_datos as $titulo_index => $data) {
        ...
        $id_subtitulo_map = []; // Mapa para almacenar ID de subtítulos
        ...
        foreach ($data['detalles'] as $detalle) {
            $id_subtitulo = $id_subtitulo_map[$detalle['subtitulo_index']] ?? null; // Obtener ID del subtítulo si existe
