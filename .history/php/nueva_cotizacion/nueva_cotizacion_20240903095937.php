...
if ($id > 0) {
    ...
    if ($stmt_empresa = $conn->prepare($sql_empresa)) {
        ...
        if ($result_empresa->num_rows == 1) {
            ...
            LEFT JOIN Tipo_Cuenta t ON cb.id_tipocuenta = t.id_tipocuenta
            ...
            $sql_last_cot = "SELECT numero_cotizacion FROM Cotizaciones WHERE id_empresa = ? ORDER BY numero_cotizacion DESC LIMIT 1";
            ...
            <fieldset>
                ...
                <div id="detalle-container"> <!-- Crea un contenedor para los detalles de la cotización. Este contenedor se utilizará para agregar dinámicamente secciones de detalle -->
                    ...
                    <div class="detalle-section"> <!-- Define una sección dentro del contenedor para agregar títulos y otros detalles relacionados con la cotización -->
                        <!-- Aquí van las secciones de detalle que se agregan dinámicamente -->
                    </div>
...
<!-- Fila para cuentas bancarias -->
            ...
            <fieldset>
                ...
                <table class="detalle-table">
                    ...
                    <tbody class="payment-info" >
                        <tr>
                            <td><input type="text" id="nombre-titular" name="nombre_titular" required></td>
                            <td><input type="text" id="rut-titular" name="rut_titular" required></td>
                            <td><input type="number" id="celular" name="celular" required></td>
                            <td><input type="email" id="email-banco" name="email_banco" required></td>
                            <td><input type="text" id="numero-cuenta" name="numero_cuenta" required></td>
                        </tr>
                        <tr class="descripcion-row" style="display: none;">
                            <td colspan="5">
                                <textarea name="condiciones_descripcion" placeholder="Ingrese detalles adicionales sobre las condiciones generales"></textarea>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <fieldset>
                ...
                <table class="detalle-table">
                    <thead>
                        <tr>
                            <th colspan="1">Mostrar información de pago:
                                <input type="checkbox" onclick="togglePaymentInfo(this)">
                            </th>
                            <th colspan="4">Forma de pago
                        ...
                        <tr class="payment-header" style="display: none;">
                            <th>N° Pago</th>
                    ...
                    <tbody class="payment-info" style="display: none;">
                        <tr>
                            <td><input type="number" id="porcentaje_adelanto_pago" name="porcentaje_adelanto_pago"></td>
                            <td><input type="checkbox" onclick="toggleDescription(this)"></td>
                            <td><input type="number" id="_pago" name="porcentaje_adelanto" min="0" max="100" required oninput="calculateAdelanto()"></td>
                            <td><input type="number" id="monto_adelanto_pago" name="monto_adelanto_pago" min="0" required readonly></td>
                            <td><input type="date" id="fecha_adelanto_pago" name="fecha_adelanto_pago" required></td>
                        </tr>
                        <tr class="descripcion-row" style="display: none;">
                            <td colspan="5">
                                <textarea name="condiciones_descripcion" placeholder="Ingrese detalles adicionales sobre las condiciones generales"></textarea>
                ...
                <?php if (!empty($bancos)): ?>
                    <tr>
                        <?php foreach ($bancos as $banco): ?>
                            <td>BANCO: <?php echo htmlspecialchars($banco['BancoNombre']); ?></td> <!-- Información del banco -->
                        <?php endforeach; ?>
                    </tr>
                    <tr>
                        <?php foreach ($bancos as $banco): ?>
                            <td>TIPO CUENTA: <?php echo htmlspecialchars($banco['TipoCuentaDescripcion']); ?></td> <!-- Tipo de cuenta -->
                        <?php endforeach; ?>
                    </tr>
                    <tr>
                        <?php foreach ($bancos as $banco): ?>
                            <td>NUMERO CUENTA: <?php echo htmlspecialchars($banco['CuentaNumeroCuenta']); ?></td> <!-- Número de cuenta -->
                        <?php endforeach; ?>
                    </tr>
                    <tr>
                        <?php foreach ($bancos as $banco): ?>
                            <td>NOMBRE: <?php echo htmlspecialchars($banco['CuentaNombreTitular']); ?></td> <!-- Nombre del titular -->
                        <?php endforeach; ?>
                    </tr>
                    <tr>
                        <?php foreach ($bancos as $banco): ?>
                            <td>RUT: <?php echo htmlspecialchars($banco['CuentaRutTitular']); ?></td> <!-- RUT del titular -->
                        <?php endforeach; ?>
                    </tr>
                    <tr>
                        <?php foreach ($bancos as $banco): ?>
                            <td>E-MAIL: <?php echo htmlspecialchars($banco['CuentaEmailBanco']); ?></td> <!-- Correo electrónico -->
                        <?php endforeach; ?>
                    </tr>
                <?php else: ?>
                    <tr>
                        <td colspan="3">No hay información bancaria disponible.</td>
                    </tr>
                <?php endif; ?>
            </table>
            <div class="container">
