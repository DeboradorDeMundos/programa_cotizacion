...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($detalles_titulo as $titulo_index => $titulo) {
        ...
        // Asignar detalles y vincularlos a subtítulos
        foreach ($detalles_cantidad[$titulo_index] ?? [] as $detalle_index => $cantidad) {
            ...
            $subtitulo_index = $_POST['subtitulo_index'][$titulo_index][$detalle_index] ?? null;
            ...
            $estructura_datos[$titulo_index]['detalles'][] = [
                ...
                'descuento' => $descuento,
                'total' => round($total, 2),
                'subtitulo_index' => $subtitulo_index // Indice del subtitulo
            ];
        }
    }
    ...
    try {
        foreach ($estructura_datos as $titulo_index => $data) {
            ...
            foreach ($data['detalles'] as $detalle) {
                ...
                $id_subtitulo = null;
                if ($detalle['subtitulo_index'] !== null) {
                    $subtitulo_index = intval($detalle['subtitulo_index']);
                    $id_subtitulo = $id_subtitulo_map[$subtitulo_index] ?? null; // Usar el índice para encontrar el ID del subtítulo
                }
