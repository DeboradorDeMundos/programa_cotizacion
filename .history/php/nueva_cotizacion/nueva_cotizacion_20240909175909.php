...
if ($id > 0) {
    ...
    e.dias_validez AS DiasValidez,  <!-- Añadido campo dias_validez -->
    f.ruta_foto
FROM e_empresa e
...
WHERE e.id_empresa = ?";
    ...
    if ($stmt_empresa = $conn->prepare($sql_empresa)) {
        ...
        if ($result_empresa->num_rows == 1) {
            ...
            $dias_validez = $row['DiasValidez'];  // Captura el valor de dias_validez
            ...
            if ($stmt_req = $conn->prepare($query_requisitos)) {
            ...
            if ($stmt_cond = $conn->prepare($query_condiciones)) {
                ...
                echo "<p>Error al preparar la consulta de condiciones generales: " . $conn->error . "</p>";
            }
