    ...
    try {
        foreach ($estructura_datos as $titulo_index => $data) {
            ...
            foreach ($data['detalles'] as $detalle) {
                // Verificar si hay subtítulo asociado
                $id_subtitulo = null; // Inicializar a null
                if (!empty($data['subtitulos'])) {
                    // Usar el primer subtítulo por defecto
                    $id_subtitulo = reset($id_subtitulo_map); // Obtener el ID del primer subtítulo
                }
