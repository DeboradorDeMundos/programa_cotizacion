...
if ($id > 0) {
    ...
    $sql = "SELECT c.*, p.Descripcion AS proyecto_descripcion, p.Codigo AS proyecto_codigo, 
                   ...
                   ds.Cantidad, ds.Descripcion AS detalle_descripcion, ds.PrecioUnitario
            FROM Cotizaciones c
            JOIN Proyectos p ON c.ID_Proyecto = p.ID
            JOIN Clientes cl ON c.ID_Cliente = cl.ID
            JOIN Vendedores v ON c.ID_Vendedor = v.ID
            JOIN DetalleServicios ds ON c.ID = ds.ID_Cotizacion
            WHERE c.ID = ?";
    ...
    if ($result->num_rows == 1) {
        ...
        $mensaje .= "<tr><td>Precio Unitario</td><td>" . htmlspecialchars($row['PrecioUnitario']) . "</td></tr>";
