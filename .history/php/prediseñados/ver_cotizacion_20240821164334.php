...
if ($id > 0) {
    ...
    $sql = "SELECT c.*, p.Descripcion AS proyecto_descripcion, p.Codigo AS proyecto_codigo, 
                   ...
                   ds.Cantidad, ds.Descripcion AS detalle_descripcion, ds.Precio_Unitario, ds.Total AS detalle_total
            FROM Cotizaciones c
            JOIN Proyectos p ON c.ID_Proyecto = p.ID
            JOIN Clientes cl ON c.ID_Cliente = cl.ID
            JOIN Vendedores v ON c.ID_Vendedor = v.ID
            LEFT JOIN detalleservicio ds ON c.ID = ds.ID_Cotizacion
            WHERE c.ID = ?";
    ...
    if ($result->num_rows == 1) {
        ...
        $mensaje .= "<tr><td>Descripción</td><td>" . htmlspecialchars($row['detalle_descripcion']) . "</td></tr>";
        $mensaje .= "<tr><td>Precio Unitario</td><td>" . htmlspecialchars($row['Precio_Unitario']) . "</td></tr>";
        $mensaje .= "<tr><td>Total</td><td>" . htmlspecialchars($row['detalle_total']) . "</td></tr>";
