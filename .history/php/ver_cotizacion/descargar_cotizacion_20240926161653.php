...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $contenido_pdf .= "3 0 obj\n<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 4 0 R /F2 5 0 R >> /XObject << /Im1 6 0 R >> >> /Contents 7 0 R >>\nendobj\n";
        ...
        // Título principal
        $contenido_pdf .= setFont('F2', 18);
        $contenido_pdf .= setColor(0.2, 0.2, 0.6);
        $contenido_pdf .= drawText(200, 750, "COTIZACION");
        ...
        // Línea separadora
        $contenido_pdf .= drawLine(50, 640, 562, 640);

        // Datos del cliente
        $contenido_pdf .= setFont('F2', 12);
        $contenido_pdf .= drawText(50, 620, "Informacion del Cliente");
        $contenido_pdf .= setFont('F1', 10);
        $contenido_pdf .= drawText(50, 605, "Nombre: " . $row['nombre_cliente']);
        $contenido_pdf .= drawText(50, 590, "RUT: " . $row['rut_cliente']);
        $contenido_pdf .= drawText(50, 575, "Direccion: " . $row['direccion_cliente']);
        $contenido_pdf .= drawText(50, 560, "Telefono: " . $row['telefono_cliente']);
        $contenido_pdf .= drawText(50, 545, "Email: " . $row['email_cliente']);

        // Datos del proyecto
        $contenido_pdf .= setFont('F2', 12);
        $contenido_pdf .= drawText(50, 515, "Detalles del Proyecto");
        $contenido_pdf .= setFont('F1', 10);
        $contenido_pdf .= drawText(50, 500, "Nombre del Proyecto: " . $row['nombre_proyecto']);

        // Total
        $contenido_pdf .= setFont('F2', 14);
        $contenido_pdf .= drawText(400, 450, "Total Final: $" . $row['total_final']);
