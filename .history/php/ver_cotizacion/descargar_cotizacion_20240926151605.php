...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (file_exists($ruta_logo)) {
            // Convertir la imagen a formato de bytes (esto requiere que la imagen esté en el mismo servidor)
            $img_data = file_get_contents($ruta_logo);
            $img_base64 = base64_encode($img_data);
            $contenido_pdf .= "q\n";
            $contenido_pdf .= "0 0 100 100 re\n"; // Tamaño de la imagen
            $contenido_pdf .= "/Im1 " . "0 0 100 100 " . "cm\n"; // Posición y escala de la imagen
            $contenido_pdf .= "BI\n";
