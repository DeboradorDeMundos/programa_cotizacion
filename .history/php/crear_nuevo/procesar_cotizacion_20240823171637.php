...
$stmt->bind_param("ssssssssssssss", $cliente_nombre, $cliente_empresa, $cliente_rut, $cliente_direccion, $cliente_lugar, $cliente_fono, $cliente_email, $cliente_cargo, $cliente_giro, $cliente_comuna, $cliente_ciudad, $cliente_tipo);
$stmt->execute();
if ($stmt->error) {
    die("Error en la ejecución de la consulta: " . $stmt->error);
}
...
foreach ($detalles_titulo as $index => $titulo) {
    ...
    $sql = "INSERT INTO Detalle_Cotizacion (id_cotizacion, id_descripcion, cantidad, descuento, neto, iva, total, total_con_descuento) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $neto = ($cantidad * $precio_unitario) - $descuento;
    $iva = $neto * 0.19;
    $total = $neto + $iva;
    ...
    $stmt->bind_param("iiddidid", $id_cotizacion, $id_descripcion, $cantidad, $descuento, $neto, $iva, $total, $total_con_descuento);
    $stmt->execute();
    if ($stmt->error) {
        die("Error en la ejecución de la consulta: " . $stmt->error);
    }
...
// Cerrar la conexión
$conn->close();
echo "Operación completada exitosamente<br>";
exit();
?>
...
<body>
    ...
    <nav>
        ...
        <ul>
            ...
            <li><a href="php/crear_nuevo/formulario_cotizacion.php">Crear Cotización</a></li><!-- Enlace para crear una nueva cotización -->
