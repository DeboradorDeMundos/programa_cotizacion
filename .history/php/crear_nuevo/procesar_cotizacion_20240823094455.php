...
if (!$encargado_id) {
    $sql_encargado = "INSERT INTO Encargados (nombre_encargado, email_encargado, fono_encargado, celular_encargado, proyecto_encargado) 
                      VALUES (?, ?, ?, ?, ?)";
    $stmt_encargado = $conn->prepare($sql_encargado);
    $stmt_encargado->bind_param("sssss", $enc_nombre, $enc_email, $enc_fono, $enc_celular, $enc_proyecto);
...
$sql_cotizacion = "INSERT INTO Cotizaciones (numero_cotizacion, fecha_emision, fecha_validez, dias_compra, dias_trabajo, trabajadores, horario, colacion, entrega, id_cliente, id_proyecto, id_empresa, id_encargado, id_vendedor) 
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt_cotizacion = $conn->prepare($sql_cotizacion);
$stmt_cotizacion->bind_param("sssssiiiiiiii", $numero_cotizacion, $fecha_emision, $fecha_validez, $dias_compra, $dias_trabajo, $trabajadores, $horario, $colacion, $entrega, $cliente_id, $proyecto_id, $empresa_id, $encargado_id, $vendedor_id);
