...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $sql_proyecto = "INSERT INTO Proyectos (Descripcion, Codigo, AreaTrabajo, Riesgo) 
        ...
        $sql_cotizacion = "INSERT INTO Cotizaciones (Fecha, Validez, TotalGeneral, ID_Cliente, ID_Proyecto, ID_Vendedor, TipoCliente, DiasCompra, DiasTrabajo, Trabajadores, Horario, Colacion, Entrega, Cargo, Giro, Comuna, Ciudad) 
                           VALUES ('$fecha_cotizacion', '$validez_cotizacion', '$total_servicio', '$cliente_id', '$proyecto_id', '$vendedor_id', '$tipo_cliente', '$dias_compra', '$dias_trabajo', '$trabajadores', '$horario', '$colacion', '$entrega', '$cargo', '$giro', '$comuna', '$ciudad')";
        $conn->query($sql_cotizacion);
        $cotizacion_id = $conn->insert_id;

        // Insertar en la tabla DetalleServicios
        $sql_detalle_servicio = "INSERT INTO DetalleServicios (Cantidad, Descripcion, Precio_Unitario, Total, ID_Cotizacion) 
                                 VALUES ('$cantidad', '$descripcion_servicio', '$precio_unitario', '$total_servicio', '$cotizacion_id')";
        $conn->query($sql_detalle_servicio);

        $conn->commit();

        header("Location: ../ver_listadover_listado.php");
        exit();
    } catch (Exception $e) {
        $conn->rollback();
