<!--
    ...
    $sql = "INSERT INTO Cuenta_Bancaria (nombre_titular, id_banco, id_tipocuenta, numero_cuenta, celular, rut_titular, email_banco)
            VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    if ($stmt === false) {
        die("Error en la preparación de la consulta: " . $conn->error);
    }
    $stmt->bind_param("siissss", $nombre_titular, $id_banco, $id_tipocuenta, $numero_cuenta, $celular, $rut_titular, $email_banco);
    $stmt->execute();
    if ($stmt->error) {
        die("Error en la ejecución de la consulta: " . $stmt->error);
    }
